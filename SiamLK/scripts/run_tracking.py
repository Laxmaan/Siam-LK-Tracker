...
@ex.config
def configs():
  checkpoint = osp.join('Logs','SiamFC','track_model_checkpoints','SiamFC-3s-color-pretrained')
  print(checkpoint)
  input_files = ','.join([x for x in glob('assets/*') if os.path.isdir(x)])
  print(input_files)
...
def main(checkpoint, input_files):
  ...
  with tf.Session(graph=g, config=sess_config) as sess:
    ...
    for video_dir in video_dirs:
      ...
      bb = [int(v) for v in re.split(',|\t',first_line.strip())]
      init_bb = Rectangle(bb[0] - 1, bb[1] - 1, bb[2], bb[3])  # 0-index in python
      print(f"video name :{video_name}")
      trajectory = tracker.track(sess, init_bb, filenames,seq_name = video_name, logdir=video_log_dir)
