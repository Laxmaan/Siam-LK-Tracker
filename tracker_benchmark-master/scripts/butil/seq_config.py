...
def save_seq_config(seq):
    ...
    configFile = open(os.path.join(src,'cfg.json'), 'w')
    json.dump(seq.__dict__,configFile, indent=2)
    configFile.close()

def load_seq_config(seqName):
    src = os.path.join(SEQ_SRC, seqName)
    configFile = open(os.path.join(src,'cfg.json'))
...
def make_seq_configs(loadSeqs):
    ...
    seqList = []
    for name in names:
        src = os.path.join(SEQ_SRC, name)
        imgSrc = os.path.join(src, 'img')
...
def download_sequence(seqName):
    file_name = os.path.join(SEQ_SRC , seqName + '.zip')

    if seqName == 'Jogging-1' or seqName == 'Jogging-2':
        url = DOWNLOAD_URL.format('Jogging')
        download_and_extract_file(url, file_name, SEQ_SRC)
        src = os.path.join(SEQ_SRC , 'Jogging')
        dst1 = os.path.join(SEQ_SRC , 'Jogging-1')
        dst2 = os.path.join(SEQ_SRC , 'Jogging-2')
        if not os.path.exists(os.path.join(dst1 , 'img')):
            shutil.copytree(os.path.join(src , 'img'), os.path.join(dst1 , 'img'))
        if not os.path.exists(os.path.join(dst2 , 'img')):
            shutil.copytree(os.path.join(src , 'img'), os.path.join(dst2 , 'img'))
        shutil.move(os.path.join(src , 'groundtruth_rect.1.txt'), os.path.join(dst1 , GT_FILE))
        shutil.move(os.path.join(src , 'groundtruth_rect.2.txt'), os.path.join(dst2 , GT_FILE))
        shutil.move(os.path.join(src , 'jogging-1.txt'), os.path.join(dst1 , INIT_OMIT_FILE))
        shutil.move(os.path.join(src , 'jogging-2.txt'), os.path.join(dst2 , INIT_OMIT_FILE))
        shutil.rmtree(src)

    elif seqName == 'Skating2-1' or seqName == 'Skating2-2':
        url = DOWNLOAD_URL.format('Skating2')
        download_and_extract_file(url, file_name, SEQ_SRC)
        src = os.path.join(SEQ_SRC , 'Skating2')
        dst1 = os.path.join(SEQ_SRC , 'Skating2-1')
        dst2 = os.path.join(SEQ_SRC , 'Skating2-2')
        if not os.path.exists(os.path.join(dst1 , 'img')):
            shutil.copytree(os.path.join(src , 'img'), os.path.join(dst1 , 'img'))
        if not os.path.exists(os.path.join(dst2 , 'img')):
            shutil.copytree(os.path.join(src , 'img'), os.path.join(dst2 , 'img'))
        ...
        download_and_extract_file(url, file_name, SEQ_SRC)
        src = os.path.join(SEQ_SRC , 'Human4')
        # dst1 = SEQ_SRC + 'Human4-1/'
        dst2 = os.path.join(SEQ_SRC , 'Human4-2')
...
def download_and_extract_file(url, dst, ext_dst):
    ...
    while True:
        ...
        f.write(buffer)
        status = r"{:d} ({:3.2f}%)".format(
            file_size_dl, file_size_dl/ file_size*100)
