...
def run_trackers(trackers, seqs, evalType, shiftTypeSet):
    tmpRes_path = RESULT_SRC.format(os.path.join('tmp','{0}').format(evalType))
    ...
    for idxSeq in range(numSeq):
        ...
        try:
            ...
            for idxTrk in range(len(trackers)):
                t = trackers[idxTrk]
                if not os.path.exists(os.path.join(TRACKER_SRC, t)):
                ...
                for idx in range(seqLen):
                    ...
                    print(os.path.join(TRACKER_SRC,t))
                    ...
                    try: r.tmplsize = butil.d_to_f(res['tmplsize'][0])
                         ...
                         print(e,end="as error")
                    r.refresh_dict()
                    seqResults.append(r)
                #end for subseqs
                if SAVE_RESULT:
                    print("\n\nSAVING IN RUN TRACKERS\n\n")
                    butil.save_seq_result(seqResults)

                trackerResults[t].append(seqResults)
        except Exception as e:
            print(e)
            print(f"failed on seq :{s}")
